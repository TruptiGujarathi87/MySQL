...
#here table_name, table_rows, information_schema.tables, table_schema is default not user defined.
...
select count(*) as dm_rowsCount from director_mapping ;
select count(*) as genre_rowsCount from genre ;
select count(*) as movie_rowsCount from movie;
select count(*) as names_rowsCount from names ;
select count(*) as ratings_rowsCount from ratings ; 
select count(*) as  rm_rowsCount from role_mapping ;



-- Q2. Which columns in the movie table have null values?
-- Type your code below:
select sum(case when id is null then 1 else 0
end) as id_nullcount,
...
end) as datepublished_nullcount,
...
end) as worldwide_gross_income_nullcount,
...
end) as production_comapny_nullcount 

from movie ;
#ANS --Country, worlwide_gross_income, languages and production_company columns has NULL values
...
SELECT YEAR, count(title) AS NoOfMovies 
FROM movie 
GROUP BY year
ORDER BY year;

#Number of movies released in each month of those years
SELECT month(date_published) AS MonthNo,
count(*) AS NoOfMovies 
FROM movie
GROUP BY MonthNo
ORDER BY MonthNo ;
...
-- Type your code below:
-- select max(genre) as CountByGenre from genre ;
...
#Thriller has maximum number of movies ;
...
-- Top 2 actors are Mammootty and Mohanlal with movie_count 8 and 5 respectively.
